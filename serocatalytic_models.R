...
Fit = fit(data = wnv_survey,
          model = model[[3]], # Choose the model from the list here or create a loop to run all of them
          chain=4,
          cores=4, 
          iter= 5000, 
          warmup = 2500,  
...
# How to compute diff in ELPD and standard error of ELPD:
# comp <- loo_compare(loo1, C2$PSIS_LOO, C3$PSIS_LOO, C4$PSIS_LOO, C5$PSIS_LOO)
