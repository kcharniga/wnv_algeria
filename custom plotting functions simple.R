seroprevalence.fit.custom.simple<- function(foi_data,  
                                            model_fit,
  ...
  chains <- rstan::extract(model_fit)
  se = 1 # sensitivity
  sp = 1 # specificity
  A <- foi_data$A
  latest_sampling_year <- max(foi_data$sampling_year)
  years <- seq(1,A)
  
  index.plot=0
 
  sorted.year = sort.int(unique(foi_data$sampling_year),index.return = TRUE)
  Y=0
  for(sampling_year in sorted.year$x ){
    Y=Y+1
      ...
      w = which(data$sampling_year ==  sampling_year, arr.ind = TRUE)
      subdat = subset(data,sub = w)
      
      # from the posterior of the model
      ll = quantile(chains$annual_foi_raw, prob = 0.025)
      ul = quantile(chains$annual_foi_raw, prob = 0.975)
      meanFit = mean(chains$annual_foi_raw)
      ...
      #p <- ggplot(DataEnvelope) + 
        #geom_polygon(aes(x, y), fill=colorribbon) + 
        #geom_line(aes(x = x, y = y), linewidth = 1, color =colorline)
      
      max.age= max(histdata$true_mean_age)
      
      p <- ggplot() +
        #geom_polygon(DataEnvelope, aes(x = , y), fill=colorribbon) + 
        
        geom_ribbon(aes(x = 0:max.age, ymin = ll, ymax = ul), fill =colorribbon) +
        geom_hline(aes(yintercept = meanFit), linewidth = 1, color =colorline) +
        ...
        theme(axis.text.x = element_text(angle = 45, hjust = 0.5, vjust = 0.85, size = 10), # 11, hjust and vjust were 0.5 both
